    ...
    this.partialExtension = {
      ...
      fixture: async (
        ...
        if (this.db.pastFixtures[fixtureKey]) {
          ...
          const success = true;// await this.revertSnapshot(pastFixture);
          if (success) {
        ...
        if (globalFixture && options.fallbackToGlobal) {
          const success = true;// await this.revertSnapshot(globalFixture);
          if (success) {
            return this.db.deployments;
          } else {
      ...
      createFixture: <T, O>(func: FixtureFunc<T, O>) => {
        ...
        return async (options?: O) => {
          ...
          if (saved) {
            const success = true;// await this.revertSnapshot(saved);
            if (success) {
              return saved.data;
            }
          }
